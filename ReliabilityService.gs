...
function dataImport_(auth,p,ss,tx){
  ...
  p.rows.forEach((r,index)=>{try{
    ...
    if(p.kind==='products'){
      ...
      const price=nonNegativeNumber_(r.price,'Үнэ'),hasCost=r.cost!==undefined&&r.cost!==null&&String(r.cost).trim()!=='',cost=hasCost?nonNegativeNumber_(r.cost,'Өртөг'):null,stock=opsQty_(nonNegativeNumber_(r.stock||0,'Үлдэгдэл')),warehouse=opsWarehouse_(ss,r.warehouse),pack=positiveNumber_(r.packSize||1,'Савлагаа'),productId=createBusinessId_('PRD');
      tx.add(COMPANY_SHEETS.PRODUCTS,{'Барааны нэр':name,ProductID:productId,'Код':clean_(r.code),'Нэгж үнэ':price,AverageCost:hasCost?cost:'',CostKnown:hasCost?'Тийм':'Үгүй','Одоогийн үлдэгдэл':stock,'Хэмжих нэгж':clean_(r.unit)||'ш','Бага үлдэгдлийн хязгаар':nonNegativeNumber_(r.threshold||0,'Бага үлдэгдэл'),'Идэвхтэй':'Тийм',PackName:clean_(r.packName),PackSize:pack});
