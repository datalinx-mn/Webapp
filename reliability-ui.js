...
(function installReliability(){
  ...
  window.renderReliability=function(data){
    ...
    if(panel)panel.innerHTML=`<h3>Бүртгэлээ шалгах</h3><div class="ops-actions">${button('Утасны бүртгэл шалгах','queue')}${button('Утасны бүртгэл татах','export-queue')}${manager()?button('Өгөгдлийн бүрэн бүтэн байдал','integrity')+button('CSV импорт','import'):''}${['manager','admin','warehouse'].includes(state.session.user.role)?button('Тооллого тулгах','stocktake'):''}</div>`;
